...
function wpex_load_scripts() {
	...
	**/
	...
	if ( get_theme_mod( 'wpex_g_font', '1' ) ) {
		wp_enqueue_style( 'wpex-google-font-source-sans-pro', 'http://fonts.useso.com/css?family=Source+Sans+Pro:200,300,400,600,700,900,200italic,300italic,400italic,600italic,700italic,900italic&subset=latin,vietnamese,latin-ext' );
		wp_enqueue_style( 'wpex-google-nunito', 'http://fonts.useso.com/css?family=Nunito:400,300,700' );
	}
...
/* if ( !function_exists( 'wpex_load_fonts_async' ) ) {
	function wpex_load_fonts_async() { ?>
		<script src="//ajax.useso.com/ajax/libs/webfont/1.4.7/webfont.js"></script>
		<script>
			WebFont.load({
				google: {
